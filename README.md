...
Open a terminal in the project folder and run one of these commands:
...
Step 1: COMPOSE
  └─ Server encrypts {sender, receiver, amount, nonce, timestamp}
  └─ RSA-OAEP + AES-256-GCM hybrid encryption
  └─ Packet injected at phone-alice (TTL=5)

Step 2: GOSSIP  
  └─ Packet hops device-to-device via Bluetooth mesh
  └─ After 1 gossip round: ALL 5 devices hold the same packet
  └─ TTL decrements per hop (prevents infinite loops)

Step 3: BRIDGE UPLOAD
  └─ phone-bridge walks outside, gets 4G
  └─ POSTs encrypted packet to /api/bridge/ingest
  └─ Backend:
    1. Hash ciphertext (SHA-256)
    2. Claim hash atomically (idempotency)
    3. Decrypt with server's RSA private key
    4. Verify freshness (timestamp within 24h)
    5. Debit sender, credit receiver (single transaction)
    6. Record in ledger

RESULT: ✅ Payment settled exactly once, even though packet reached backend


BASEMENT (NO WIFI, NO 4G):
┌─────────────────────────────────────────────────┐
│ You (Alice)           Other people              │
│ ├─ Phone (offline)    ├─ phone-stranger1        │
│ ├─ ₹5000 balance      ├─ phone-stranger2        │
│ ├─ wants to send ₹500 ├─ phone-stranger3       │
│ │  to Bob             ├─ phone-bridge (special) │
│ └────────────────────┼─────────────────────────┘
│                      │
│   Bluetooth Mesh (NO INTERNET NEEDED)
│   Packet hops phone → phone → phone
│                      │
│                      ▼
│          ┌─ phone-bridge walks outside
│          │  (person carrying it leaves basement)
│          │  Gets 4G signal ────→ Server
│          │
│          └─ THEN posted to backend
└─────────────────────────────────────────────────┘

cd "d:\Final year project\UPI_Without_Internet-main" ; 
